...
class CompanyInformationBloc
  ...
  /// here use cases for all the bloc
...
// here for choose location of company
  ...
  /// here for search om map

  void searchPlaces(String query) async {
    emit(LoadingSearchGoogleMapState());
    ...
    emit(LoadedSearchGoogleMapState(
      markers: markers,
    ));
  }
  ...
  CompanyInformationBloc({
    ...
    on<CompanyInformationEvent>((event, emit) async {
      ///
      /// her for google map search
      ///
      if (event is GoogleMapSearchEvent) {
        searchPlaces(event.text);
      }
      ...
      if (event is GetAllCompanyTypeEvent) {
        ...
        String? companyInformation =
            sharedPreferences.getString('companyInformation');
        ...
        failureOrGetAllCompanyType.fold((failure) async {
          ...
          if (companyInformation != null) {
            ...
            selectedCountryId = sharedPreferences.getInt(
              'countryId',
            );
            ...
            markerLocation !=
                LatLng(double.parse(data['addressLatitude']),
                    double.parse(data['addressLongitude']));
      ...
      if (event is GetAllCitiesEvent) {
        ...
        final failureOrGetAllCities =
            await getAllCitiesUseCase(event.countryId);
      ...
      if (event is SelectedValueForCitiesDropDownButtonEvent) {
        ...
        emit(SelectedValueForCitiesDropDownButtonState(
      ...
      if (event is UploadImageEvent) {
        ...
        failureOrUploaded.fold((failure) {
      ...
      if (event is AddCompanyEvent) {
        ...
        final failureOrAddedCompany =
            await addCompanyUseCase(event.addCompanyEntity);
        failureOrAddedCompany.fold((failure) {
