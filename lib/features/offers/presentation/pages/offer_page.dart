...
class OfferPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocProvider(
        create: (context)=>di.sl<OfferBloc>()..add(const GetAllTenderEvent(sortValue: 'aliMo'))..add(GetAllCitiesOfferEvent()),
        child: BlocConsumer<OfferBloc,OfferState>(
          listener: (context,state){
            ...
            if(state is LoadedAcceptedOfferState){
              SnackBarMessage().showSnackBar(message: 'Offer Accepted Successfully', backgroundColor: primaryColor, context: context);
              OfferBloc.get(context).add(const GetAllTenderEvent(sortValue: 'aliMo'));
